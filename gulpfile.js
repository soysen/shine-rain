(function(){
    ...
    gulp.task('franchisee-scripts', function (cb) {
        gulp.src(f7.jsFiles)
            .pipe(tap(function (file, t){
                addJSIndent (file, t);
            }))
    ...
    gulp.task('admin-styles-ios', function (cb) {
        var cbs = 0;
    ...
    gulp.task('admin-styles-material', function (cb) {
        var cbs = 0;
    ...
    gulp.task('franchisee-styles-ios', function (cb) {
        var cbs = 0;
        ['framework7.ios.less', 'framework7.ios.rtl.less', 'framework7.ios.colors.less'].forEach(function(lessFilePath, b){
            lessFilePath = paths.source.styles.ios + lessFilePath;
            gulp.src([lessFilePath])
                ...
                .pipe(header(f7.banner, { pkg : f7.pkg, date: f7.date, theme: 'iOS Theme' }))
                .pipe(gulp.dest('build/franchisee/css'))
                .pipe(connect.reload())
                .on('end', function () {
                    cbs ++;
                    if (cbs === 3) cb();
    ...
    gulp.task('franchisee-styles-material', function (cb) {
        var cbs = 0;
        ['framework7.material.less', 'framework7.material.rtl.less', 'framework7.material.colors.less'].forEach(function(lessFilePath, b){
            lessFilePath = paths.source.styles.material + lessFilePath;
            gulp.src([lessFilePath])
    ...
    gulp.task('demo-app', function (cb) {
        gulp.src(paths.source.root + '**/*.jade')
            .pipe(jade({
                pretty: true,
                locals: {
        ...
        gulp.src(paths.source.root + 'admin/css/app.css')
            .pipe(gulp.dest('build/admin/css'));
        
        gulp.src(paths.source.root + 'franchisee/js/app.js')
            .pipe(gulp.dest('build/franchisee/js'));

        gulp.src(paths.source.root + 'franchisee/css/app.css')
            .pipe(gulp.dest('build/franchisee/css'));

        gulp.src(paths.source.root + 'admin/img/**/*.*')
            .pipe(gulp.dest('build/admin/img'))
            .pipe(connect.reload());

        gulp.src(paths.source.root + 'franchisee/img/**/*.*')
            .pipe(gulp.dest('build/franchisee/img'))
            .pipe(connect.reload());
        cb();
    });

    gulp.task('build', ['admin-scripts', 'franchisee-scripts', 
                'admin-styles-ios', 'admin-styles-material', 
                'franchisee-styles-ios', 'franchisee-styles-material', 'demo-app'
        ], function (cb) {
        cb();
    });
    ...
    gulp.task('watch', function () {
        ...
        // Demo App
        gulp.watch([
            paths.source.root + 'admin/*.jade', 
            paths.source.root + 'admin/js/*.*', 
            paths.source.root + 'admin/css/*.*', 
            paths.source.root + 'admin/img/**/*.*',
            paths.source.root + 'franchisee/*.jade', 
            paths.source.root + 'franchisee/js/*.*', 
            paths.source.root + 'franchisee/css/*.*', 
            paths.source.root + 'franchisee/img/**/*.*'
        ], ['demo-app']);
